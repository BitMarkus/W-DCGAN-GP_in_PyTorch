...
class Train():
    ...
    def __init__(self, device, dataloader):
        ...
        self.generate_att_maps_epochs = setting["generate_att_maps_epochs"]
    ...
    def _plot_attention_maps(self, real_images, epoch):
        ...
        with torch.no_grad():
            # Get attention maps
            attention_maps = self.netC.get_attention_maps(real_images)           
            ...
            img = (real_images[0].permute(1, 2, 0).cpu().numpy() + 1) / 2
            ...
            plt.tight_layout()
            plt.savefig(f"{self.pth_samples}attention_epoch_{epoch}.png")
            plt.close()
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            with tqdm(self.dataloader, unit="batch") as tepoch:
                ...
                # Plot attention maps
                if((epoch + 1) % self.generate_att_maps_epochs == 0 and self.use_attention):
                     self._plot_attention_maps(real_images, epoch)
